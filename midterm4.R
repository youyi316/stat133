...
# [B] : the number of rolls, how many times to roll
# and returns:
...
dice_sum <- function(k=2, B=100){
  ...
  a=rep(1:6, k)
  for (i in 1:B){
    sum[i] = sum(sample(a, k, replace=T))
  }
  dsum = sum
  return(dsum)
}
...
hotelCal.split <- function(hotelCal){
    new.hotelCal = tolower(hotelCal)  
    new.hotelCal = gsub("'", "", new.hotelCal)
    new.hotelCal = gsub("[[:punct:]]+", "", new.hotelCal)
    splitted = unlist(strsplit(new.hotelCal, " "))
    return(splitted)
  }
