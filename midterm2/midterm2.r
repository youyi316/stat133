...
set.seed(344)
m <- matrix(c(rexp(100, rate=3)), nrow=10, ncol=10, byrow=F)
#m <- <your code here>
...
#t <- <your code here>
z<- infants[which(infants$parity==1), 12]
t <- table(infants[which(infants$parity==1), 5], infants[which(infants$parity==1), 12])

# [2 pt]
# Calculate [mw], the average birthweight ($bwt) of all babies whose were full term, i.e. gestation equal or more than 259 days.
mw <- mean(infants[which(infants$gestation>=259), 2])
...
iris$Colour[iris$Species=="setosa"]="red"
