...
name <- "You Yi"
github.name <- "youyi316"
email.address <- "youyi316@berkeley.edu"
...
y <- list()
for(i in 1:10){y[[i]]=seq(i,1000,by=i)}
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
v <- sample(1:6, size=100, replace=T)
...
set.seed(344)
m <- matrix(c(rnorm(100, mean=8)),nrow=10, ncol=10, byrow=F)
...
lm <- lm(mtcars$qsec~mtcars$hp)
plot(mtcars$qsec~mtcars$hp)
abline(a=coef(lm(mtcars$qsec~mtcars$hp))[1], b=coef(lm(mtcars$qsec~mtcars$hp))[2])
...
plot(WorldBank$life.expectancy~WorldBank$fertility.rate, pch=".", col=WorldBank$region,
...
hist(WorldBank$GDP.per.capita.Current.USD[WorldBank$lending=="IDA"],  breaks = 45, 
     main = paste("Histogram of" , "GDP"),  xlab = "GDP")
...
par(mfrow=c(2,3))
sapply(rain, function(x) hist(x, prob=T))
...
GenNorm <- function(mean=0, sd=1, n=1000, plot.hist="TRUE"){
  ...
  else if (plot.hist!="TRUE") stop("No histogram")
return(p)
}
...
standardizeVar <- function(m, cols=TRUE){
  for (i in 1:ncol(m)){
    for (j in 1:nrow(m)) {
  if(cols =="TRUE"){
    m1[j,i]=(m[j,i]-mean(m[,i]))/sd(m[,i])  
    }
  else if (cols =="FALSE") {
    m1[j,i]=(m[j,i]-mean(m[j,]))/sd(m[j,]) 
  }
  }    
  }
  return(m1)
...
PermDiff <- function(cases, controls, k=5000){
 m=length(controls)
  all=cases+controls
  cases1=sample(all, size=k, replace=T)
...
if(game == sample(1:19, size=1, replace=T)){counter=counter+1}
n.jackpot=counter
  return(n.jackpot)
}
...
position <- grep(length(unlist(strsplit(dna[],""))) %%3 != 0, dna)
dna2 <- gsub("dna[position]", "", dna)
